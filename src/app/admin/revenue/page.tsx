...
const KPICard = ({ title, value, change, trend, delay }: any) => (
    <motion.div
        ...
        transition={{ delay }}
        className="glass-card p-6 rounded-2xl border border-white/[0.05]"
    >
...
const TransactionRow = ({ id, user, type, amount, status, date }: any) => (
    <div className="flex items-center justify-between p-4 hover:bg-white/[0.02] transition-colors border-b border-white/[0.05] last:border-0 group">
        <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-xs text-white/40">{type} • {date}</p>
            </div>
        </div>
        <div className="text-right">
            <p className="text-sm font-mono text-white">{amount}</p>
    ...
    return (
        <div className="h-full flex flex-col gap-8">
            <header className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-light text-white">Financial Overview</h2>
                    <p className="text-white/40 text-sm mt-1">Revenue streams, MRR, and transaction history.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1 min-h-0">
                ...
                <div className="lg:col-span-2 glass-panel rounded-3xl border border-white/[0.05] p-6 flex flex-col">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-lg font-light text-white">Revenue Growth</h3>
                        <div className="flex gap-2">
                            {['1M', '3M', '6M', 'YTD'].map(period => (
                                <button key={period} className="px-3 py-1 rounded-lg text-xs hover:bg-white/10 text-white/40 hover:text-white transition-colors">
                                    {period}
                                </button>
                            ))}
                        </div>
                    ...
                    {/* CSS Bar Chart */}
                    <div className="flex-1 flex items-end justify-between gap-2 px-4 pb-4 border-b border-white/[0.05] relative">
                        {/* Grid Lines */}
                    ...
                    <div className="flex justify-between mt-2 text-[10px] text-white/30 uppercase tracking-widest px-2">
                        <span>Jan</span><span>Dec</span>
                    </div>
                </div>

                {/* Recent Transactions */}
                <div className="glass-panel rounded-3xl border border-white/[0.05] flex flex-col overflow-hidden">
                    <div className="p-6 border-b border-white/[0.05]">
                        <h3 className="text-lg font-light text-white">Recent Payouts</h3>
                    </div>
